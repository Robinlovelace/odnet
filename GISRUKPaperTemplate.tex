...
A well-known pitfall when studying origin-destination (OD) data aggregated over areal units is generating OD pairs that resemble real movements. Rather than deriving single OD flows from areal unit centroids, we present and report on a technique whereby, for any given pair of areal units, a rich set of OD flows is generated through sampling (or jittering) candidate point locations using the underlying road network at each areal unit as input data. We provide technical details around collecting and sampling from road network data and discuss exciting opportunities for parameterising the sampling process to generate flows that might be characteristic of particular stated travel behaviours.
