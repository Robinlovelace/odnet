...
desire_lines_joined = purrr::map_dfr(1:length(network_filenames), function(i) {
  ...
  desire_lines$type = names(network_filenames)[i]
  desire_lines
})
desire_lines_joined = desire_lines_joined %>% 
  mutate(type_factor = factor(type, levels = names(network_filenames)))

tm_shape(zones, bbox = tmaptools::bb(zones, ext = 0.8)) +
  tm_borders(alpha = 0.5) +
  tm_shape(network %>% sample_n(1000)) +
  tm_lines(col = "red", lwd = 0.2) +
  tm_shape(desire_lines_joined) +
  tm_lines(lwd = "bicycle", title.lwd = "Number of trips", lwd.legend = c(10, 50)) +
  tm_facets(by = "type_factor", free.coords = FALSE) +
