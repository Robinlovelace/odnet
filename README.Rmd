...
p1 <- ggplot() +
  geom_sf(data=zones, size=.1, alpha=.3) +
  geom_sf(data=network$geometry[sel_net], colour="#67000d", size=.1)+
  geom_sf(data=desire_lines$geometry[sel_net], alpha=.2, size=.7)+
  coord_sf(crs=st_crs(zones), datum=NA)+
  ggtitle("title here")
...
p2 <- ggplot() +
  geom_sf(data=zones, size=.1, alpha=.3) +
  geom_sf(data=network$geometry[sel_net], colour="#67000d", size=.1)+
  geom_sf(data=read_sf("output_max10k.geojson"), alpha=.2, size=.7)+
  coord_sf(crs=st_crs(zones), datum=NA)+
  ggtitle("title here")
...
p3 <- ggplot() +
  geom_sf(data=zones, size=.1, alpha=.3) +
  geom_sf(data=network$geometry[sel_net], colour="#67000d", size=.1)+
  geom_sf(data=read_sf("output_max50k.geojson"), alpha=.2, size=.7)+
  coord_sf(crs=st_crs(zones), datum=NA)+
  ggtitle("title here")
...
```{r rnets, fig.cap="Route network results."}
...
d$r_sqaured = NA
j = 1
rnet_list = lapply(1:length(routes_names), function(j) {
  i = routes_names[j]
  message(i)
  routes = sf::read_sf(i)
